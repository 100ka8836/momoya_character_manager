...
function insertCharacterToDatabase($pdo, $data, $groupId, $sourceUrl)
{
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            (name, occupation, birthplace, degree, mental_disorder, age, sex, address, description, family, insanity, injuries, scar, income, cash, deposit, personal_property, real_estate, mythos_tomes, artifacts_and_spells, encounters, note, chatpalette, portrait_url, source_url, group_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $data['name'],
